  ...
  const searchLabels = async () => {
    try {
      ...
      if (response.length === 0) {
        toast.info('No existen etiquetas con ese nombre, puedes crearla si deseas', 3000)
        setIsEmpty(false)
      } else {
        setLabels(response)
      }
        ...
        <div className='flex gap-5 justify-center items-end '>
          <div>
            ...
            <Textinput
              ...
              type='text'
              register={register}
              placeholder='Buscar etiqueta y seleccionarla'
        ...
        {etiquetas.length > 0
          ? <div className='w-full flex flex-wrap items-center text-sm font-medium mt-5'>
            {etiquetas.map((etiqueta) => {
              return (
                <span className='bg-base-100 border  text-md font-medium me-2 group font-mono  p-2 rounded-lg dark:bg-purple-900 dark:text-purple-300 flex uppercase' key={etiqueta.id}>@{etiqueta.nombre}
                  <button className='ml-2 text-black group-hover:flex hidden dark:text-white justify-center items-center transition duration-150 ease-out hover:ease-in' onClick={() => deletLabel(etiqueta.id)}>
