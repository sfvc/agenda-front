...
export const ContactSelect = ({ handleContact, oldContacts }) => {
  ...
  const addContact = (e) => {
    ...
    if (!exists) {
      ...
      toast.warning('El contacto ya existe en la lista')
      setFind([])
    }
    setSearch('')
  }
  ...
  return (
    <>
      <div className='flex flex-col justify-center items-start my-5'>
        ...
        {contacts.length > 0 && (
          <div className='w-full flex flex-wrap items-center text-sm font-medium mt-5 gap-2'>
            {contacts.map((contactos) => (
              <span key={contactos.id} className='bg-base-100 border text-md font-medium me-2 group font-mono p-2 rounded-lg dark:bg-purple-900 dark:text-purple-300 flex uppercase'>
