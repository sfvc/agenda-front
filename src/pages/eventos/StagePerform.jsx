...
import { searchContactName, searchContactFunction } from '@/services/contactService'
...
export const StagePerform = () => {
    ...
    // Busca los contactos con los parametros recibidos , solo por nombre y apellido
    ...
    const addContactFunction = async () => {
        try {
            ...
            // Filtramos los que ya existen en la lista de invitados
            ...
            setContact([]);  // Limpiamos la lista de contactos
            setSearch("");    // Limpiamos el campo de búsqueda
        } catch (error) {
            console.error("Error al buscar contactos:", error);
        }
    };

    // Agrega un contacto a la lista de invitados (si no existe ya)
    const addContact = (e) => {
    ...
    //borra de la lista al invitado seleccionado
    const deleteGuest = (e) => {
                        ...
                        <Card>
                            <div className="w-full  gap-5">
                                <div class="flex">
                                    ...
                                    <div className='w-1/2 mx-6 mb-6 relative'>
                                        ...
                                        {contacts.length > 0 ? <div className="w-full text-sm font-medium text-gray-900 bg-white  border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white absolute">
                                    ...
                                    <div className='w-1/2 mx-6 mb-6 flex items-end justify-around'>
                                        <SelectForm title="Agregar Grupo" options={functions} onChange={(e) => setTags(e.target.value)} />
                                        <Button
                                            type='submit'
                                            text={'Agregar Grupo'}
                                                                ...
                                                                ? (invitados.map((contacto) => {
                                                                    return (
                                                                        <tr key={contacto.id}>
                                                                            ...
                                                                            <td className='table-td flex justify-start gap-2'>{contacto.funcion}</td>
                                                                            <td >
